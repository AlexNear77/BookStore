...
usersCtrl.signup = async (req , res) => {
   const errors = [];
   const {nombre , email, password, nombreTitular,numeroDeTarjeta,confirm_password} = req.body;
   ...
   if (errors.length > 0) {
      ...
      if (emailUser) { // si encontraste algo ose correo
         ...
         const newUser = new User({nombre, email, password,nombreTitular,numeroDeTarjeta});
 ...
 usersCtrl.signin = passport.authenticate('local',{
    failureRedirect: '/users/signin', // cuando algo falle lo direcionamos ahi
    successRedirect: '/productos', //si se loguea correctamente va a ahi
